...
myseeds <- seq(1, 9, by = 1)

for(ms in 1:length(myseeds)){
  set.seed(myseeds[ms])
  ...
  system.time(out <- ssdm(YY = data$Y, XX = data$X, ZZ = data$Z, Niter = Niter, burn = burn, 
  ...
  truth1 <- betaincluded1
  betaincluded1[which(as.vector(ppithetaX) > .5)] <- 1
  truth1[which(as.vector(data$thetax) != 0)] <- 1
  
  plot(betaincluded1, type="h", ylim = c(0, 1), xlab = expression(paste("index for ", theta[x])),
  ...
  truth2 <- betaincluded2
  betaincluded2[which(as.vector(ppithetaZ) > .5)] <- 1
  truth2[which(as.vector(data$thetaz) != 0)] <- 1
  
  plot(betaincluded2, type="h", ylim = c(0, 1), xlab = expression(paste("index for ", theta[z])), 
  ...
  plotrix::plotCI(1:(P*Q*J), quant[, 2], li=quant[, 1], ui=quant[, 3], 
                  scol = col, xlab = "Interaction Index", 
                  ylab = expression(b[xz]), xaxt="n", ylim=c(-2.0, 2.0))
  axis(1, at = seq(0, (P*Q*J), by = 100), las=2)
  abline(h=0, col = "red", lty =2)
  plot(c(bixzincluded), type="h", ylim=c(0, 1), xlab = expression(paste("index for ", b[xz])),
       ylab = expression(paste("Inclusion ", b[xz])))
  truth3 <- rep(0, length(c(bixzincluded)))
  truth3[which(c(data$inter_xz != 0))] <- 1
  ...
  plot(betaincluded_l, type="h", ylim=c(0, 1), xlab = expression(paste("index for ", alpha^{0})),
       ylab = expression(paste("PPI ", alpha^{0})))
  ...
  plot(betaincluded_nl, type="h", ylim=c(0, 1), xlab = expression(paste("index for ", alpha, "*")),
       ylab = expression(paste("PPI ", alpha, "*")))
  points(which(data$nlind == 1), rep(1, sum(data$nlind)), type="p", pch=20, cex = 1, col="red")
  ...
  plotrix::plotCI(1:(Q*Q*J), quant2[, 2], li=quant2[, 1], ui=quant2[, 3], 
                  scol = col, xlab = "Interaction Index", 
                  ylab = expression(b[zz]), xaxt="n", ylim=c(-2.0, 2.0))
  axis(1, at = seq(0, (Q*Q*J), by = 100), las=2)
  abline(h=0, col = "red", lty =2)
  plot(c(bizzincluded), type="h", ylim=c(0, 1), xlab = expression(paste("index for ", b[zz])),
       ylab = expression(paste("Inclusion ", b[zz])))
  truth4 <- rep(0, length(c(bizzincluded)))
  truth4[which(c(data$inter_xz != 0))] <- 1
